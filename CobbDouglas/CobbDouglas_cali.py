...
for sim_idx in range(10):
    ...
    sub_folder = 'sim_{}_zero_{}'.format(sim_idx, bench_zero)
    sub_cali_dir = '{}/{}'.format(cali_dir, sub_folder)

    if not os.path.exists(sub_cali_dir):
        os.makedirs(sub_cali_dir)

    with open('{}/zero_{}/sim_path_{}.pickle'.format(log_dir, bench_zero, sim_idx), 'rb') as fp:
    ...
    for alph_idx in range(alpha_arr.shape[0]):
        ...
        sub_dir = '{}/zero_{}/{}'.format(log_dir, bench_zero, sub_f)
        ...
        for t in range(1, time_horizon):
            c, s, w = np.nonzero(reg_pi[t, scr_list[-1], worker_list[-1]])
            conn_list.append(c.copy())
            scr_list.append(s.copy())
            worker_list.append(w.copy())

        pa_num = len(worker_list[-1])
        ...
        for p in range(sim_path.shape[0]):
            for t in range(time_horizon):
                r_value[p, 2*t] = sim_path[p, 2*t]/firmtype_num
                r_value[p, 2*t+1] = sim_path[p, 2*t+1]/workertype_num
        ...
        for p in range(sample.shape[0]):
            for t in range(time_horizon):
                # it's score
                ot_value[p, 2*t] = sample[p, 2*t]/firmtype_num
                ot_value[p, 2*t+1] = sample[p, 2*t+1]/workertype_num
