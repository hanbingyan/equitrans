...
for gs_idx in range(11):
    ...
    for sim_idx in range(10):
        ...
        sub_folder = 'gs_{}_group_{}_sim_{}_zero_{}_simprop_{}'.format(int(gs[gs_idx]), w_split_n, sim_idx,
        ...
        path = '{}/gs_{}_group_{}_zero_{}_simprop_{}'.format(log_dir, int(gs[gs_idx]), w_split_n,
                                                             bench_zero, SIMDATA_PROP)

        with open('{}/sim_path_{}.pickle'.format(path, sim_idx), 'rb') as fp:
        ...
        for alph_idx in range(alpha_arr.shape[0]):
            ...
            # sub_dir = '{}/gs_{}_group_{}/{}'.format(log_dir, int(gs[gs_idx]), w_split_n, sub_f)

            with open('{}/{}/ot_pi.pickle'.format(path, sub_f), 'rb') as fp:
                pi = pickle.load(fp)

            with open('{}/{}/ot_init.pickle'.format(path, sub_f), 'rb') as fp:
            ...
            # thres_idx = np.argmax(np.cumsum(prob_arr[sorted_idx]) > 0.6)
            ...
            if alph_idx%25 == 0:
                print('Alpha', alpha, 'GS', gs[gs_idx])
